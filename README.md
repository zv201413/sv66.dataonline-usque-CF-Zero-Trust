...
- **加密模式 (推荐)**：
  ...
  ./manage.sh start
  ```
  该模式会通过 `gost` 额外嵌套一层 Shadowsocks 加密，适合在公网环境使用。
- **SOCKS5 直连模式 (不推荐)**：
  ```bash
  ./manage.sh start-socks
  ```
  该模式直接暴露 SOCKS5 端口，无加密。仅建议在内网测试或对安全性无要求时使用。

- **端口建议**: 建议输入 `35001 - 35999` 之间的数字。
- **自检逻辑**: 脚本会自动检查端口是否被占用以及 TLS 握手是否成功。

### 第三步：获取节点链接
- **加密模式**: 屏幕会输出绿色的 `ss://` 链接，直接导入即可。
- **SOCKS5 模式**: 屏幕会输出 IP 和端口，在软件中手动添加 SOCKS5 协议节点即可。
...
**主要改进**：本项目保留其生成明文 SOCKS5 链接功能的基础上通过 `gost` 增加了“本机到主机”这一段的 Shadowsocks 加密封装，极大提升了在受限网络环境下的安全性与稳定性。
## 声明
